...
void COX_Line_Fit(
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        ...
        MatrixXd points_rep(N * numLines, 2);

	for(int i=0; i<N; i++) {
		for(int k=0; k<numLines; k++) {
			points_rep(i*numLines+k, 0) = meas_world(0, i);
			points_rep(i*numLines+k, 1) = meas_world(1, i);
		}
	}
	
	//MatrixXd points_rep_int = meas_world.topRows(2);
        //MatrixXd points_rep = points_rep_int({},VectorXi::LinSpaced(N*numLines,0,N-1)).transpose();
